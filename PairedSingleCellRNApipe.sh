...
for R1_SRC in $(find . -type f -name '*R1_trimmed.fq.gz'); do
    ...
    eval kb count "$R1_SRC" "$R2_SRC" -i Homo_sapiens.GRCh38.cdna.all.index -x 10XV3 -g t2g.txt -t 8 --cellranger -o "$OUT" &> "$LOG"
done
...
for R1_SRC in $(find . -type f -name '*R1_trimmed.fq.gz'); do
    R2_SRC=${R1_SRC//R1_trimmed.fq.gz/R2_trimmed.fq.gz}
    OUTR1="${R1_SRC%R1_trimmed.fq.gz}_mapped1"
    LOGR1="${R1_SRC%R1_trimmed.fq.gz}_mapped1.log"
    OUTR2="${R2_SRC%R2_trimmed.fq.gz}_mapped2}"
